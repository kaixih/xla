...
TEST_F(CollectiveOpsTestE2E, Bar) {
  absl::string_view kModuleReplicatedStr = R"(
HloModule module, entry_computation_layout={(bf16[3,128,128], bf16[3,1,32,128], bf16[], bf16[])->bf16[3,128,128]}, allow_spmd_sharding_propagation_to_parameters={false,false,false,false}, num_partitions=4
...
while_cond {
  param = (s32[], bf16[3,128,128], bf16[3,1,32,128], bf16[], bf16[]) parameter(0)
...
while_body {
  ...
  r = bf16[1,32,128] bitcast(dynamic-slice.k)
  a = bf16[1,32,128] add(r, r), control-predecessors={constant.2559}

  qa = f8e4m3fn[1,32,128] convert(a)
  dqa = bf16[1,32,128] convert(qa)
  a_scale = bf16[] get-tuple-element(param), index=3
  a_scales = bf16[1,32,128] broadcast(a_scale), dimensions={}
  dqa_unscaled = bf16[1,32,128] multiply(dqa, a_scales)

  ag = bf16[1,128,128] all-gather(dqa_unscaled), channel_id=1, use_global_device_ids=true, dimensions={1}, replica_groups={{0,1,2,3}}
  ...
  ROOT tuple = (s32[], bf16[3,128,128], bf16[3,1,32,128], bf16[], bf16[]) tuple(add.230, dynamic-update-slice.35, get-tuple-element.k, a_scale, ma_scale), control-predecessors={a}
}
...
ENTRY entry {
  ...
  s0 = bf16[] parameter(2)
  s1 = bf16[] parameter(3)
  tuple = (s32[], bf16[3,128,128], bf16[3,1,32,128], bf16[], bf16[]) tuple(c0, p0, p1, s0, s1)
  while = (s32[], bf16[3,128,128], bf16[3,1,32,128], bf16[], bf16[]) while(tuple), condition=while_cond, body=while_body
...
TEST_F(CollectiveOpsTestE2E, Foo) {
  absl::string_view kModuleReplicatedStr = R"(
HloModule pjit_foo, entry_computation_layout={(s32[4,2]{1,0})->s32[4,2]{1,0}}, allow_spmd_sharding_propagation_to_output={false}, num_partitions=4
...
ENTRY main.25 {
  ...
  ROOT all-gather.14 = s32[4,2]{1,0} all-gather(dynamic-slice.13), channel_id=1, use_global_device_ids=true, replica_groups={{0,1,2,3}}, dimensions={0}, metadata={op_name="pjit(foo)/jit(main)/pmap(<lambda>)/all_gather[all_gather_dimension=0 axis_name=i axis_index_groups=None axis_size=4 tiled=False]" source_file="/home/tmp/move_cp_post_layout/psum.py" source_line=31}
